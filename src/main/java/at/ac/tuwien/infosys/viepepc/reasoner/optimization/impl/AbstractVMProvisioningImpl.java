...
public class AbstractVMProvisioningImpl extends AbstractProvisioningImpl {
    ...
    protected VirtualMachine startNewVm(OptimizationResult result, VMType vmType, ContainerConfiguration containerConfiguration) {
        ...
        vmsShouldBeStarted.addAll(inMemoryCache.getWaitingForExecutingProcessSteps().stream().map(processStep -> processStep.getScheduledAtContainer().getVirtualMachine()).collect(Collectors.toList()));
